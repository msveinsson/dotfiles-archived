	...
	local windowChooser = hs.chooser.new(function(choice)
		...
		if v then
			...
			if moveToCurrent then
				...
				hs.spaces.moveWindowToSpace(v, hs.spaces.activeSpaceOnScreen(hs.screen.mainScreen()))
...
function obj:bindHotkeys(mapping)
	local def = {
		all_windows = function()
			self:selectWindow(false, false)
		end,
		all_windows_current = function()
			self:selectWindow(false, true)
		end,
		app_windows = function()
			self:selectWindow(true, false)
		end,
	}
	hs.spoons.bindHotkeysToSpec(def, mapping)
end

--- CUSTOM change to add hyper application multible windowChoices
